...
def print_summary_table(all_results):
    ...
    print(f"\n{'Experiment':<30} {'Jobs×Robots':<15} {'Network(ms)':<12} {'Hungarian(ms)':<14} {'QP(ms)':<10} {'Total(ms)':<12}")
    print("-"*100)
    ...
    for name in ['small', 'medium', 'large']:
        if name in all_results:
            ...
            net_ms = timing['network_forward']['mean'] * 1000
            hung_ms = timing['hungarian']['mean'] * 1000 if timing['hungarian'] else 0
            qp_ms = timing['qp']['mean'] * 1000 if timing['qp'] else 0
            total_ms = timing['total_decision']['mean'] * 1000
            
            print(f"{EXPERIMENT_CONFIGS[name]['name']:<30} {config_str:<15} {net_ms:<12.4f} {hung_ms:<14.4f} {qp_ms:<10.4f} {total_ms:<12.4f}")
    ...
    if 'small' in all_results and 'large' in all_results:
        ...
        print(f"Problem Size Increase: {(1000*500)/(100*5):.0f}x")
