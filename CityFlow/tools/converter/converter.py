...
def calc_edge_compass_angle(edge):
    ...
    # Reverse the edge to compute the incoming direction (more intuitive).
...
def calc_edge_compass_angle_no_modify(edge):
    north_ray = sympy.Ray((0, 0), (0, 1))
    # We need angles for all edges, so do not reverse.
...
def node_to_intersection(node,tls_dict,edge_dict):
    ...
    intersection = {
        ...
        "width": 0,  # WARNING: intersection width is undefined; using 15 by convention
        ...
        "virtual": _is_node_virtual(node)  # classify dead_end as virtual
    }
    ...
    for k, v in connections_group.items():
        ...
        # Add the angle to north for the incoming direction
            ...
            if TRUE_CORRECTION_lane:
                for end_inx,end_lane in enumerate(reversed(end_road._lanes)):
                    ...
                    path = {
                        ...
                        # TODO: maybe use [start-lane end, intersection point, end-lane start].
                        "points": [start_point, end_point]  # WARNING: "flying" mode
                    }
...
def get_final_roads(net):
    ...
    for edge in edges:
        ...
        lane_template = {
            "width": 4,
            "maxSpeed": 11.111  # WARNING: ignore max speed
        }
        if TRUE_CORRECTION_lane:
