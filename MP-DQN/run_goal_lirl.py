...
# -*- coding: utf-8 -*-
"""
LIRL Agent for Goal Environment
===============================
基于LIRL算法在Goal环境上训练
...
class ActionProjection:
    ...
    Goal环境动作空间:
    - 3个离散动作: kick_to(0), shoot_goal_left(1), shoot_goal_right(2)
    - 连续参数: kick_to需要2个参数(x,y), shoot_goal_left/right各需要1个参数
    """
    ...
    def __init__(self, action_space, use_qp=True):
        ...
        """
        self.use_qp = use_qp
        self.num_actions = 3  # kick_to, shoot_goal_left, shoot_goal_right
        
        # 动作参数的维度
        self.action_param_sizes = [2, 1, 1]  # kick_to:2, shoot_left:1, shoot_right:1
        self.action_param_offsets = [0, 2, 3, 4]  # 累计偏移量
...
class LIRLAgent:
    ...
    def __init__(self, state_size, action_param_size, action_space=None, 
                 hidden_layers=(128, 64), lr_actor=1e-4, lr_critic=1e-3, 
                 gamma=0.95, tau=0.005, buffer_size=100000, batch_size=128, 
                 use_action_projection=True, seed=None):
        
        self.state_size = state_size
        self.action_param_size = action_param_size  # 4: kickto(2) + shoot_left(1) + shoot_right(1)
        self.gamma = gamma
        self.tau = tau
        ...
        # 动作参数范围（Goal环境）
        # kickto: x in [-1,1], y in [-1,1]
        # shoot_left: power in [-1,1]
        # shoot_right: power in [-1,1]
        self.action_param_min = np.array([-1., -1., -1., -1.])
        self.action_param_max = np.array([1., 1., 1., 1.])
    ...
    def act(self, state, add_noise=True):
        ...
        if self.use_action_projection:
            ...
            # 根据选择的动作提取对应的参数
            if action == 0:  # kick_to
                act_param = np.clip(action_params[:2], -1, 1)
            elif action == 1:  # shoot_goal_left
                act_param = np.clip(action_params[2:3], -1, 1)
            else:  # shoot_goal_right
                act_param = np.clip(action_params[3:4], -1, 1)
...
@click.option('--initial-memory-threshold', default=128, help='Number of transitions required to start learning.', type=int)
...
def run(seed, episodes, evaluation_episodes, batch_size, gamma, initial_memory_threshold,
        ...
        scale_actions, reward_scale, use_action_projection, layers, save_dir, title):
    ...
    action_param_size = 4  # kickto(2) + shoot_left(1) + shoot_right(1)
    ...
    for i in range(episodes):
        ...
        for j in range(max_steps):
            ...
            # 存储经验
            agent.step(state, action_probs, action_params, r, next_state, terminal)
        ...
        if (i + 1) % 100 == 0:
            ...
            print('{0:5s} R:{1:.5f} P(S):{2:.4f} Eps:{3:.4f}'.format(
                str(i + 1), avg_reward, success_rate, agent.epsilon))
